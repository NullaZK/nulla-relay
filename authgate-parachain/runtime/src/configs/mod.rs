...
parameter_types! {
    ...
    pub const FeeAssetId: AssetId = AssetId(RelayLocation::get());
}
...
parameter_types! {
    pub MessageQueueServiceWeight: Weight = Perbill::from_percent(35) * MAXIMUM_BLOCK_WEIGHT;
}
...
// ── AuthGate Application Pallet Config ───────────────────────────────────

parameter_types! {
    /// Sovereign account of ProofHub (para 2000) on this chain: Sibling(2000).
    pub ProofHubSovereign: AccountId = {
        use polkadot_parachain_primitives::primitives::Sibling;
        use sp_runtime::traits::AccountIdConversion;
        Sibling::from(2000u32).into_account_truncating()
    };
    /// Sovereign account of ScanProofHub (para 2002) on this chain: Sibling(2002).
    pub ProofHubSovereign2: AccountId = {
        use polkadot_parachain_primitives::primitives::Sibling;
        use sp_runtime::traits::AccountIdConversion;
        Sibling::from(2002u32).into_account_truncating()
    };
}

impl pallet_access_keys::Config for Runtime {
    type RuntimeEvent = RuntimeEvent;
    type AdminOrigin = EnsureRoot<AccountId>;
    type ProofHubSovereign = ProofHubSovereign;
    type ProofHubSovereign2 = ProofHubSovereign2;
}
